...
router.get("/", (req, res) => {
  ...
  if (search) {
    if (searchType === "title") {
      query = `
        SELECT p.id, p.title, u.nickname, p.created_at, p.view_count
      ...
      `;
      countQuery =
        "SELECT COUNT(*) as cnt FROM posts p WHERE p.title LIKE ? OR p.content LIKE ?";
      ...
      `;
      countQuery =
        "SELECT COUNT(*) as cnt FROM posts p JOIN users u ON p.user_id = u.id WHERE u.nickname LIKE ?";
      params = [`%${search}%`];
    }
  } else {
    query = `
...
router.get("/:id", (req, res) => {
  ...
  const comments = db
    .prepare(
      `SELECT c.id, c.content, c.created_at, c.user_id, u.nickname
...
router.get("/:id/edit", (req, res) => {
  ...
  if (!post) return res.redirect("/posts");
  if (post.user_id !== user.id) return res.redirect(`/posts/${postId}`);
...
router.post("/:id/edit", (req, res) => {
  ...
  if (!post || post.user_id !== user.id) return res.redirect("/posts");
...
router.post("/:id/delete", (req, res) => {
  ...
  if (!canDelete) return res.redirect(`/posts/${postId}`);
